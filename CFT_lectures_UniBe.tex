...
Taking into account Lorentz symmetry, one can also establish that the Wightman function $W(x)$ can only depend on the Lorentz-invariant distance $x^2$, although there are subtleties associated with light-like points (see below).
...
\begin{equation}
	\frac{1}{k^2} \left[ 1 + \gamma \log(k^2) + \ldots \right]
	\approx (k^2)^{-1 + \gamma}.
\end{equation}
...
since scaling dimensions must be bounded, there must be an operator that is annihilated by $K^\mu$:
...
verify that Wightman 2-point function readily satisfies this differential equation

however: one could have 2-point function of two distinct operators:
give condition that they have identical scaling dimensions
...
also: for spinning operators, gives relation between  different polarizations

give explicitly example for vector

exercise: do the same for 2-index symmetric, traceless tensor

exercise show that scaling dim of $T^{\mu\nu}$ is $d$

conclude that $\Delta = d$ for the energy-momentum tensor 
(also proves that $T^{\mu\nu}$ is a primary operator)
