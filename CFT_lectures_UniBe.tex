...
\begin{equation}
	\Delta = \frac{d-2}{2}.
\end{equation}
$\sigma(x)$ is an infinitesimal scale factor that satisfies $\partial^2 \sigma = 0$.
...
Taking into account Lorentz symmetry, one can also establish that the 2-point function in position space must only depend on the invariant distance $(x - y)^2$ between the two points, although there are subtleties associated to possible light-cone crossings (see below). In momentum space, the delta function in $p + q$ also multiplies a function $\rho$ of the only Lorentz invariant $q^2$ (or equivalently $p^2$).
When combined with the condition that only states of positive energy can exist, we deduce (up to factors of $2 \pi$ that are purely conventional)
\begin{equation}
	\bra{0} \widetilde{\phi}(p) \widetilde{\phi}(q) \ket{0}
	= (2\pi)^{d+1} \delta^d(p + q)
	\theta\left( q^0 - \left| \vec{q} \right| \right) \rho(-q^2).
	\label{eq:spectraldensity}
\end{equation}

This result is in fact raising an important point (also part of the Wightman axioms): in spite of their names, Wightman functions are \emph{not} functions but rather distributions. The above 2-point function computes in fact the overlap between the two states (note that the Hermitian conjugation flips the sign of momenta)
\begin{equation}
	\widetilde{\phi}(q) \ket{0}
	\qquad\text{and}\qquad
	\widetilde{\phi}(-p) \ket{0},
\end{equation}
and therefore one would expect that it computes the norm of this state in the limit $p \to q$. However, this limit is clearly discontinuous, or the norm of the state infinite.

The resolution of this issue is that the objects $\phi(x)$ and its Fourier transform $\widetilde{\phi}(p)$ are not operators, but rather \emph{operator-valued distributions}. In other words, $\phi(x) \ket{0}$ is \emph{not} a state of the theory, as it has in fact infinite norm (see example below). Formally, these operator-valued distributions only make sense when they are integrated against test functions, defining
\begin{equation}
...
\begin{equation}
	...
	\widetilde{\phi}(p),
\end{equation}
where $f$ and $\widetilde{f}$ are Schwartz-class test functions (i.e.~functions decaying faster than any power at infinity).
When acting on the vacuum, these smeared states give well-defined states, with finite norms. For instance, we have
\begin{align}
	...
	\label{eq:norm}
\end{align}
Test functions won't appear in these lectures anymore. For physicists, this is mostly an annoyance that we prefer to avoid. However, it is important to know that there exists a mathematically rigorous way of dealing with Wightman functions.
For one thing, this gives a proper justification of why it is always fine to take the Fourier transform way and back between the position- and momentum-space representation, at least as long as we are dealing with Wightman functions.

% more details needed on this

\subsection{Spectral representation}

The norm \eqref{eq:norm} is also giving away important information: is is positive if and only if the function $\rho$ is positive for any $-q^2 > 0$. This is not an accident: the function $\rho$ is in fact the \emph{spectral density} encountered in standard quantum field theory textbooks, where we can often find it in the form
\begin{equation}
	\bra{0} \phi(x) \phi(y) \ket{0}
...
\end{equation}
\begin{exercise}
	Verify that this expression matches eq.~\eqref{eq:spectraldensity}
	upon Fourier transforming in $x$ and $y$.
\end{exercise}
The spectral representation is also used to construct the time-ordered correlation function
\begin{equation}
...
\begin{equation}
	...
	e^{i k \cdot (x - y)}
	\frac{-i \rho(\mu^2)}
	{k^2 + \mu^2 - i \varepsilon}
\end{equation}
to be understood in the limit $\varepsilon \to 0_+$.
This is the \emph{Källen-Lehmann} representation for the time-ordered 2-point function.
%
\begin{exercise}
	Derive the Källen-Lehmann representation starting from
	eq.~\eqref{eq:spectralrepresentation}.
%	\begin{align*}
%		\bra{0} & \phi(x) \phi(y) \ket{0}
%		\nonumber \\
%		&= \int \frac{d^dk}{(2\pi)^d}
%		\int\limits_0^\infty d\mu^2 \,
%		e^{i k \cdot (x - y)} \theta(k^0) 
%		\left[ \frac{i}{k^2 + \mu^2 + i \varepsilon}
%		- \frac{i}{k^2 + \mu^2 - i \varepsilon} \right]
%		\rho(\mu^2).
%	\end{align*}
\end{exercise}
%
In the theory of a free, massive scalar field, the spectral density obeys
\begin{equation}
...
\end{equation}
is the ordinary propagator for a massive field.
...
discuss connection with path integral formalism: time-ordered correlation functions
...
since generator $D$ commutes with $M^{\mu\nu}$, it is always possible to diagonalize it at point $x = 0$
\begin{equation}
	\left[ D, \phi(0) \right] = 
\end{equation}
...
equivalent to transformation rule for the field in the Lagrangian?

compute conformal generators in momentum-space representation

impose and get result for spectral density

keep coefficient arbitrary so far

use Källen-Lehman representation to get time-ordered 2-point function

fix the coefficient to be one there
...
interpretation:
operator vs.~field;
often said that CFT is a theory of massless excitations: true, but also of all masses


similar to Wigner construction of particle of mass $m$: use reference momentum


then Fourier transform back to position space, and get ordinary propagator;
verify that the constant is positive for all $\Delta$

fix coefficient to be one in position space


consequence: 
one-point functions vanish unless operator has scaling dimension 0?

only identity!

exercise show that scaling dim of $T^{\mu\nu}$ is $d$



\subsection{Conformal symmetry}

$P^\mu$ raises scaling dimension, $K^\mu$ lowers it

if bounded, there must be an operator that is annihilated by $K^\mu$:

declare this to be a \emph{primary} operator, with
\begin{equation}
...
get general case
\begin{equation}
	\left[ K^\mu, \phi(x) \right] = \ldots
\end{equation}


special condition on 2-point function of primary operators: that they have identical scaling dimensions

use this to diagonalize space of operators (usually possible)

this proves a posteriori many assumptions made before



also: for spinning operators, gives more unitarity bounds

give explicitly result for vector

exercise: do the same for 2-index symmetric, traceless tensor
